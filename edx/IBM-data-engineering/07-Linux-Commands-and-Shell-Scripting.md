...
The operating system, OS, is the software that manages hardware and resources. It allows interaction with the hardware via higher level abstractions. UNIX is a family of closed source OS, while Linux is an open source family of OS which were created to provide an alternative to UNI. The key features of Linux include: being free and open source, multi-user, multi-tasking, and portability.

There are five layers to a Linux system: UI (allows interaction with the machine) -> Application Layer (system demons, shells, user apps, tools) -> Operating System (responsible for job scheduling and time tracking, detecting errors and file management) -> Kernel (manages memory, processing, hardware, and security) -> Hardware

The shell is an OS level application, like Bash and ZSH, that interprets commands. A terminal is an application that allows interacting with the shell. Commands are sent through the terminal to the shell, where the command  passes through the OS, via the kernel, to the hardware to process the command, and then the result is sent back through the layers.
...
The shebang is what tells the executor which interpreter to use:

`#! /usr/bin/bash`
or 
`#! /usr/bin/python3`

Filters are shell commands/programs, taking input from standard input and sent to standard output, transforming the input data into output data. Such as wc, cat, more, head, sort, grep etc. They can be chained together using the pipe command.
...
Shell variables are created by using `variablename=value`, then to display this use `echo $variablename`. Unset variablename deletes the variable. 

Metacharacters include: # for comments, ; separates commands on the same line, * represents any number of characters in a filename pattern, e.g. ls /bin/ba*, ? represents a single character in a pattern. When quoting with double quotes any $name patterns are evaluated as the variable value, unless the $ is escaped, \$. When using single quotes everything inside is treated literally.

I/O redirection to a file: > redirects output to the file (overwriting), >> appends to output file, 2> redirects error to the file, 2>> append error to file. < redirection to pass file contents to standard input.
